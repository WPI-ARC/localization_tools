...
from valve_localization_panel_msgs.msg import *
...
class ValveLocalizer:
    ...
    def updateFromPanel(self, data):
        ...
        if (data.Hands == PanelUpdate.PLANNER_BOTH):
            self.valve_status.hands = self.valve_status.BOTH
        elif (data.Hands == PanelUpdate.PLANNER_LEFT):
            self.valve_status.hands = self.valve_status.LEFT
        elif (data.Hands == PanelUpdate.PLANNER_RIGHT):
            self.valve_status.hands = self.valve_status.RIGHT
        elif (data.Hands == PanelUpdate.USER_BOTH):
            self.valve_status.hands = self.valve_status.USER_BH
        elif (data.Hands == PanelUpdate.USER_LEFT):
        ...
        if (data.Command == 15):
            ...
            self.valve_status.lastPlan = self.valve_status.GETREADY
            data.Info = self.valve_status.lastReturn
            updatePublisher.publish(data)
        elif (data.Command == 1): 
            self.valve_status.color = "SOLID"
            self.call_planner(self.valve_status.GRASP) 
            self.valve_status.lastPlan = self.valve_status.GRASP
            data.Info = self.valve_status.lastReturn
            updatePublisher.publish(data)
        elif (data.Command == 2): 
            self.valve_status.color = "SOLID"
            self.call_planner(self.valve_status.TURN) 
            self.valve_status.lastPlan = self.valve_status.TURN
            data.Info = self.valve_status.lastReturn
            updatePublisher.publish(data)
        elif (data.Command == 3): 
            self.valve_status.color = "SOLID"
            self.call_planner(self.valve_status.UNGRASP) 
            self.valve_status.lastPlan = self.valve_status.UNGRASP
            data.Info = self.valve_status.lastReturn
            updatePublisher.publish(data)
        elif (data.Command == 4): 
            self.valve_status.color = "SOLID"
            self.call_planner(self.valve_status.FINISH) 
            self.valve_status.lastPlan = self.valve_status.FINISH
            data.Info = self.valve_status.lastReturn
            updatePublisher.publish(data)
        elif (data.Command == 5): 
            self.valve_status.color = "SOLID"
            ...
            if self.valve_status.lastReturn == "NoError":
                if self.valve_status.lastPlan == self.valve_status.GETREADY:
                    data.State = "READY"
                elif self.valve_status.lastPlan == self.valve_status.TURN:
                    data.State = "TURNING"
    ...
    def call_planner(self, stage):
        ...
        try:
            ...
            self.valve_status.lastReturn = error_code
...
##################################
    ...
    def planning_cb(self, feedback):
        ...
        if handle == planning_getready:
            self.valve_status.color = "SOLID"
            self.valve_status.turn_amount = 30
            self.call_planner(self.valve_status.GETREADY)
